...
import { L0_USDC, SRS, CE_USDC, WH_USDC } from 'config/coins'
import useActiveWeb3React from 'hooks/useActiveWeb3React'
import { useMemo } from 'react'
import { useSarcoinPrice } from '@pancakeswap/utils/useSarcoinPrice'
...
export const useStableSarcoinAmount = (_amount: number): number | undefined => {
  // const sarcoinBusdPrice = useSarcoinPrice()
  // if (sarcoinBusdPrice) {
  //   return multiplyPriceByAmount(sarcoinBusdPrice, amount)
  // }
  return undefined
}

export { useSarcoinPrice }

export const usePriceSarcoinUsdc = () => {
  const chainId = useActiveChainId()
  const sarcoinPrice = useTokenUsdcPrice(SRS[chainId])
  return useMemo(() => (sarcoinPrice ? new BigNumber(sarcoinPrice) : BIG_ZERO), [sarcoinPrice])
}
