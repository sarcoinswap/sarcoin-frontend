  ...
  const farmsList = useCallback(
    (farmsToDisplay: DeserializedFarm[]): FarmWithStakedValue[] => {
      const farmsToDisplayWithAPR: FarmWithStakedValue[] = farmsToDisplay?.map((farm) => {
        ...
        const { sarcoinRewardsApr } = isActive
          ? getFarmApr(new BigNumber(farm.poolWeight ?? 0), sarcoinPrice, totalLiquidity, regularSarcoinPerBlock ?? 0)
          : { sarcoinRewardsApr: 0 }

        const lpRewardsApr = lpRewardsAprs?.[farm.lpAddress?.toLowerCase()] ?? 0

        return { ...farm, apr: sarcoinRewardsApr, lpRewardsApr, liquidity: totalLiquidity }
      })

      return filterFarmsByQuery(farmsToDisplayWithAPR, query)
    },
    [query, isActive, sarcoinPrice, regularSarcoinPerBlock, lpRewardsAprs],
  )
  ...
  return (
    <FarmsContext.Provider value={providerValue}>
      ...
      <Page title={t('Farms')}>
        ...
        <NoSSR>
          {viewMode === ViewMode.TABLE ? (
            <Table
              ...
              sarcoinPerBlock={sarcoinPerBlock}
            />
          ) : (
        ...
        <StyledImage src="/images/decorations/3dpan.png" alt="Sarcoin illustration" width={120} height={103} />
      </Page>
