...
const FarmTable: React.FC<React.PropsWithChildren<ITableProps>> = ({
  ...
  const generateRow = (farm) => {
    ...
    const farmSarcoinPerSecond =
      farm.poolWeight && sarcoinPerBlock ? (Number(farm.poolWeight) * Number(sarcoinPerBlock)) / 1e8 : 10
    ...
    const row: RowProps = {
      ...
      multiplier: {
        multiplier: farm.multiplier,
        rewardSarcoinPerSecond: true,
        farmSarcoinPerSecond:
          farmSarcoinPerSecond === 0
            ? '0'
            : farmSarcoinPerSecond < 0.000001
            ? '<0.000001'
            : `~${farmSarcoinPerSecond.toFixed(6)}`,
        totalMultipliers,
      },
