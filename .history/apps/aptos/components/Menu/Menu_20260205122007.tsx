...
import { useSarcoinPrice } from 'hooks/useStablePrice'
...
export const Menu = ({ children }: { children: ReactNode }) => {
  ...
  const { data: sarcoinPrice } = useSarcoinPrice()
  ...
  return (
    <UIMenu
      ...
      langs={languageList}
      sarcoinPriceUsd={sarcoinPrice ? Number(sarcoinPrice) : undefined}
      // @ts-ignore
      subLinks={activeMenuItem?.hideSubNav || activeSubMenuItem?.hideSubNav ? [] : activeMenuItem?.items}
      activeSubItem={activeSubMenuItem?.href}
      toggleTheme={toggleTheme}
      buySarcoinLabel={t('Buy SRS')}
      buySarcoinLink="https://aptos.sarcoinswap.finance/swap?outputCurrency=0x159df6b7689437016108a019fd5bef736bac692b6d4a1f10c941f6fbb9a74ca6::oft::SarcoinOFT"
    >
      {children}
    </UIMenu>
  )
