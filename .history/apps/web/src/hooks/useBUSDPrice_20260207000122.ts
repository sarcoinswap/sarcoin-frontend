...
export function useStablecoinPrice(
  ...
  const isSarcoin = chainId && currency && CAKE[chainId] && currency.wrapped.equals(CAKE[chainId])

  const isStableCoin = currency && stableCoin && currency.wrapped.equals(stableCoin)

  const shouldEnabled = currency && stableCoin && enabled && currentChainId === chainId && !isSarcoin && !isStableCoin
  ...
  const price = useMemo(() => {
    ...
    if (isSarcoin && sarcoinPrice) {
      return new Price(
        currency,
        stableCoin,
        1 * 10 ** currency.decimals,
        getFullDecimalMultiplier(stableCoin.decimals).times(sarcoinPrice.toFixed(stableCoin.decimals)).toString(),
      )
    }
