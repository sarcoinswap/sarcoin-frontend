...
const FarmCard: React.FC<React.PropsWithChildren<FarmCardProps>> = ({
  ...
  const { totalMultipliers, getFarmSarcoinPerSecond } = useFarmV2Multiplier()

  const farmSarcoinPerSecond = getFarmSarcoinPerSecond(farm.poolWeight)
  ...
  const lpLabel = farm.lpSymbol && farm.lpSymbol.replace(/sarcoin/gi, '')
