...
const FarmInfo: React.FunctionComponent<React.PropsWithChildren<FarmInfoProps>> = ({
  ...
  const { lpSymbol, token, quoteToken, multiplier, stakedPositions, unstakedPositions, pendingSarcoinByTokenIds } = farm
  ...
  const hasEarningTokenIds = useMemo(
    () =>
      Object.entries(pendingSarcoinByTokenIds)
  ...
  const totalEarnings = useMemo(
    () =>
      +formatBigInt(
        Object.values(pendingSarcoinByTokenIds).reduce((total, vault) => total + vault, 0n),
        4,
      ),
    [pendingSarcoinByTokenIds],
  )
  ...
  const earningsBusd = useMemo(
    () => new BigNumber(totalEarnings).times(sarcoinPrice).toNumber(),
