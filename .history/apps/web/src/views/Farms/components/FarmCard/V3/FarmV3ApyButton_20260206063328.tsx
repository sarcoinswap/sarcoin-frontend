...
function FarmV3ApyButton_({ farm, existingPosition, isPositionStaked, tokenId }: FarmV3ApyButtonProps) {
  ...
  const sarcoinAprFactor = useMemo(
    () =>
      new BigNumber(farm.poolWeight)
        .times(farmV3.sarcoinPerSecond)
        .times(365 * 60 * 60 * 24)
        .times(sarcoinPrice)
        .div(
    ...
    [
      ...
      farm.poolWeight,
      farmV3.sarcoinPerSecond,
      isPositionStaked,
    ],
  )

  const positionSarcoinApr = useMemo(
    () =>
      existingPosition
        ? outOfRange
          ? 0
          : new BigNumber(existingPosition.liquidity.toString()).times(sarcoinAprFactor).div(depositUsdAsBN).toNumber()
        : 0,
    [sarcoinAprFactor, depositUsdAsBN, existingPosition, outOfRange],
  )
  ...
  const sarcoinApr = +farm.sarcoinApr

  const displayApr = getDisplayApr(sarcoinApr, lpApr)
  const sarcoinAprDisplay = sarcoinApr.toFixed(2)
  const positionSarcoinAprDisplay = positionSarcoinApr.toFixed(2)
  ...
  const estimatedAPR = useMemo(() => {
    return (parseFloat(sarcoinAprDisplay) * USER_ESTIMATED_MULTIPLIER + parseFloat(lpAprDisplay)).toLocaleString('en-US', {
      maximumFractionDigits: 2,
    })
  }, [sarcoinAprDisplay, lpAprDisplay])
  const boostedAPR = useMemo(() => {
    return (parseFloat(positionSarcoinAprDisplay) * boostMultiplier + parseFloat(lpAprDisplay)).toLocaleString('en-US', {
      maximumFractionDigits: 2,
    })
  }, [positionSarcoinAprDisplay, lpAprDisplay, boostMultiplier])
  ...
  const positionDisplayApr = getDisplayApr(+positionSarcoinApr, lpApr)
  const positionBoostedDisplayApr = getDisplayApr(boostMultiplier * positionSarcoinApr, lpApr)
    ...
    <>
      ...
      <ul>
        <li>
          {t('Farm APR')}:{' '}
          <b>
            {canBoosted && <>{parseFloat(sarcoinAprDisplay) * USER_ESTIMATED_MULTIPLIER}% </>}
            <Text
      ...
      <br />
      <Text>
        {t('Calculated using the total active liquidity staked versus the SRS reward emissions for the farm.')}
      </Text>
      {canBoosted && (
        <Text mt="15px">
          {t('bSRS only boosts Farm APR. Actual boost multiplier is subject to farm and pool conditions.')}
        </Text>
      )}
    ...
    <>
      ...
      <ul>
        <li>
          {t('Farm APR')}:{' '}
          <b>
            {isBoosted && <>{(parseFloat(positionSarcoinAprDisplay) * boostMultiplier).toFixed(2)}% </>}
            <Text
              display="inline-block"
              bold={!isBoosted}
              style={{ textDecoration: isBoosted ? 'line-through' : 'none' }}
            >
              {positionSarcoinAprDisplay}%
            </Text>
          </b>
        </li>
        <li>
    ...
    <>
          ...
          <ApyLabelContainer alignItems="center" style={{ textDecoration: 'initial' }} onClick={roiModal.onOpen}>
            {outOfRange ? (
              <TooltipText decorationColor="failure" color="failure" fontSize="14px">
                {positionSarcoinApr.toLocaleString('en-US', { maximumFractionDigits: 2 })}%
              </TooltipText>
            ) : (
              <>
      ...
      {sarcoinPrice && sarcoinAprFactor && (
        <RoiCalculatorModalV2
          ...
          sarcoinPrice={sarcoinPrice.toFixed(3)}
          sarcoinAprFactor={sarcoinAprFactor.times(isBoosted ? boostMultiplier : 1)}
          prices={prices}
