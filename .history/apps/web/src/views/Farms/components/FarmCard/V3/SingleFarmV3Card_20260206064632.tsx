...
import { BSarcoinV3CardView } from '../../YieldBooster/components/bCakeV3/CardView'
import {
...
interface SingleFarmV3CardProps {
  ...
  token: Token
  quoteToken: Token
  pendingSarcoinByTokenIds: IPendingCakeByTokenId
  onDismiss?: () => void
...
> = ({
  ...
  const totalEarnings = useMemo(
    () => +formatBigInt(pendingSarcoinByTokenIds[position.tokenId.toString()] || 0n, 4),
    [pendingSarcoinByTokenIds, position.tokenId],
  )

  const earningsBusd = useMemo(() => {
    return new BigNumber(totalEarnings).times(sarcoinPrice.toString()).toNumber()
  }, [sarcoinPrice, totalEarnings])
          ...
          <>
            ...
            <RowBetween flexDirection="column" alignItems="flex-start" flex={1} width="100%">
              <FarmV3HarvestAction
                ...
                disabled={!pendingSarcoinByTokenIds?.[position.tokenId.toString()] ?? true}
                userDataReady
