...
const GITHUB_ENDPOINT = 'https://raw.githubusercontent.com/sarcoinswap/airdrop-v3-users/master'
...
const V3AirdropModal: React.FC = () => {
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const { sarcoinAmountInWei, nft1, nft2 } = v3ForSCResponse?.[account?.toLowerCase()] || {}
      ...
      if (!v3MerkleProofsResponse) {
        ...
        const receipt = await fetchWithCatchTxError(() =>
          v3AirdropContract.write.claim([sarcoinAmountInWei, nft1, nft2, proof], {
  ...
  const textDisplay = (): string => {
    if (hasPart1 && hasPart2) {
      return t(
        'You have received an exclusive v3 Legendary and v3 Early Supporter NFT, as well as a SRS airdrop for participating in the SarcoinSwap v3 Launch campaign. Claim your reward now and enjoy the benefits of being an early adopter!',
      )
    }
    if (hasPart1) {
      return t(
        'You have received an exclusive v3 Legendary NFT and SRS airdrop for participating in the PancakeSwap v3 Launch campaign. Claim your reward now and enjoy the benefits of being an early adopter!',
      )
    }
    return t(
      'You have received an exclusive v3 Early Supporter NFT and CAKE airdrop for participating in the SarcoinSwap v3 Launch campaign. Claim your reward now and enjoy the benefits of being an early adopter!',
    )
  }
