...
import { getDisplayFarmSarcoinPerSecond } from '../getDisplayFarmSarcoinPerSecond'
...
const FarmCard: React.FC<React.PropsWithChildren<FarmCardProps>> = ({
  ...
  const { totalRegularAllocPoint, sarcoinPerBlock } = useFarms()

  const totalMultipliers = totalRegularAllocPoint ? (Number(totalRegularAllocPoint) / 100).toString() : '0'

  const farmSarcoinPerSecond = getDisplayFarmSarcoinPerSecond(farm.poolWeight?.toNumber(), sarcoinPerBlock)
  ...
  const earnLabel = farm.dual ? farm.dual.earnLabel : t('SRS + Fees')
